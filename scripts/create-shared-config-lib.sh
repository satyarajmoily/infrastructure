...
# This script creates a shared configuration library for both agents

set -e

echo "📚 Creating Shared Configuration Library"
echo "========================================"
...
# Create the shared configuration library
cat > "$INFRA_DIR/shared_config.py" << 'EOF'
"""
...
echo "✅ Created shared configuration library: $INFRA_DIR/shared_config.py"

echo ""
echo "🎯 Next Steps:"
echo "1. Copy this file to both agent projects"
echo "2. Update agent settings to use centralized config"
echo "3. Remove duplicate configurations"
echo ""

# Copy to both agent projects
if [ -d "$ROOT_DIR/coding-ai-agent/src" ]; then
    cp "$INFRA_DIR/shared_config.py" "$ROOT_DIR/coding-ai-agent/src/"
    echo "✅ Copied to coding-ai-agent/src/"
fi

if [ -d "$ROOT_DIR/devops-ai-agent/src" ]; then
    cp "$INFRA_DIR/shared_config.py" "$ROOT_DIR/devops-ai-agent/src/"
    echo "✅ Copied to devops-ai-agent/src/"
fi

echo ""
echo "📚 Shared Configuration Library created successfully!"
echo "🔧 Both agents can now use: from shared_config import config" 